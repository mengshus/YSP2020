'''
    Refer to https://github.com/Lyken17/pytorch-OpCounter
    Modified from thop.profile
    The results are shown like as follows:
    	layer name or total
...
def profile_prune(model, inputs, double_macs=False, custom_ops=None, verbose=False, \
    ...
    for name, m in model.named_modules():  # layer name (...conv, fc, etc.)
        ...
        if not prune and show_layer and (isinstance(m, nn.modules.conv._ConvNd) or isinstance(m, nn.Linear)):  # conv or fc
            print(name)
            print('    params: {:.0f}    flops: {:.0f}'.format(params[name], ops[name]))
            print('')
    ...
    if not prune:
        print('total params: {:.0f}    total flops: {:.0f}'.format(total_params, total_ops))
    ...
    for name, m in model.named_modules():  # layer name (...conv, fc, etc.)
        ...
        if show_layer and (isinstance(m, nn.modules.conv._ConvNd) or isinstance(m, nn.Linear)):  # conv or fc
            print(name)
            print('    params rmn/tot: {:.0f}/{:.0f}    flops rmn/tot: {:.0f}/{:.0f}'.
                format(params_rmn[name], params[name], ops_rmn[name], ops[name]))
            print('')
    ...
    print('total:\n    params rmn/tot: {:.0f}/{:.0f} ({:.2f}x)    flops rmn/tot: {:.0f}/{:.0f} ({:.2f}x)'.
        format(total_params_rmn, total_params, total_params/total_params_rmn, \
            total_ops_rmn, total_ops, total_ops/total_ops_rmn))
